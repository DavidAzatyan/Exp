...
public class ReadDataFromTelemetry {
    ...
    public final PerCustomerIdTrigramIndices perCustomerIdTrigramIndices = createOrUseDefault(
            this.perCustomerIdTrigramIndices, new PerCustomerIdTrigramIndices(c ->
                    new IdTrigramIndices(c, "test", db, CUSTOMER_ID_MANAGER, ID_MANAGERS)));

    @Nonnull
    private <T> T createOrUseDefault(@Nullable T t, T defaultValue) {
        if (t != null) {
            return t;
        }
